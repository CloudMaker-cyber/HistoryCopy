...
- **批量删除** —— 进入多选模式勾选多条记录,一键删除
- **图片文字识别** —— 图片卡片点"识别"即可提取图中文字,支持一键复制(离线识别,数据不出电脑)
- **搜索** —— 关键词实时过滤,命中文字高亮显示
- **图片预览** —— 图片卡片可点开大图预览
...
│   ├── cleanup.py     # 到期清理
│   ├── autostart.py   # 开机自启
│   ├── ocr.py         # 图片文字识别(RapidOCR,离线)
│   └── ui/            # 界面(PySide6)
├── docs/              # 需求/技术/设计/计划/使用文档
├── devlog/            # 开发日志
└── requirements.txt   # 依赖清单(PySide6 / Pillow / rapidocr / onnxruntime)
```
...
- 打包后的 exe 因内置 OCR 识别模型,体积约 330MB(正常)。
